...
// ── Import names ──
function findTableByLabel(label) {
    return TABLES.find((t) => t.label.toLowerCase() === label.toLowerCase());
}
function detectExportFormat(lines) {
    ...
    for (let i = startIdx; i < lines.length; i++) {
        ...
        if (isExport && parts.length >= 4) {
            ...
            if (!firstName && !lastName)
                continue;
            const guest = { id: generateId(), firstName, lastName, allergy };
            state.guests.push(guest);
            const table = findTableByLabel(tableLabel);
            if (table && !isNaN(seatNumber)) {
                // Remove any existing assignment for this seat
                state.assignments = state.assignments.filter((a) => !(a.tableId === table.id && a.seatNumber === seatNumber));
...
function populateSwapDropdowns() {
    const selA = document.getElementById("swap-a");
    const selB = document.getElementById("swap-b");
